...
function buildDraft(
  ...
  // A machine transcript the clinician then CHANGED must say so — "transcribed in the cloud" alone
  // over edited text presents the clinician's own words as whisper output. Only a real machine
  ...
  const transcriptHop = cloudTranscript
    ? `transcribed in demo mode (cloud)${editedSuffix}`
    : audioLeft
